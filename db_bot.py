...
print("SetupSQLScript", setupSqlScript)
...
strategies = {
    ...
    "single_domain_double_shot": (setupSqlScript +
                   " Which song appears in the greatest number of playlists? " +
                   " \nSELECT s.SongID, s.Title, COUNT(ps.PlaylistID) AS PlaylistCount \nFROM Song s \nJOIN PlaylistSong ps ON s.SongID = ps.SongID \nGROUP BY s.SongID, s.Title \nORDER BY PlaylistCount DESC \nLIMIT 1; " +
                   commonSqlOnlyRequest)
}

questions = [
    "Which user has created the most playlists?",
    "What are the top 3 most popular songs based on user interactions?",
    "Which artist has the most songs in the database?",
    "Which playlists contain only songs from a single genre?",
    "Which users have liked more than one song?",
    "Are there any albums that do not have any songs associated with them?",
    "Which song appears in the greatest number of playlists?",
    "Which users have interacted with songs from the most different genres?",
    "What is the average number of songs per playlist?",
    "Which artists have songs that have never been added to any playlist?"
]
